...
{
    public partial class XLpizza : Form
    {
        public string kadi;
        public string sifre;
    
        public XLpizza(string kadii, string sifree)
        {
            InitializeComponent();
            kadi = kadii;
            sifre = sifree;
        ...
        private void btn_siparis_Click(object sender, EventArgs e)
        {
            ...
            if (icecek == null || icecek == "Promosyon İçecek İstemiyorum" || icecek == "" || icecek == " ")
            {
                ...
                con.connection.Open();
                string query = $"insert into Sepetim(Ürün,PizzaSecim,KenarTipi,Promosyonİcecek,Fiyat,DukkanAdi,KullaniciID,Siparismi)values('XL Pizza','{pizzasecim}','{kenar}','{icecek}',130,'Dominos','{kID}','false')";
            ...
            else
            {
                ...
                con.connection.Close();
                con.connection.Open();
                string query = $"insert into Sepetim(Ürün,PizzaSecim,KenarTipi,Promosyonİcecek,Fiyat,DukkanAdi,KullaniciID,Siparismi)values('XL Pizza','{pizzasecim}','{kenar}','{icecek}',150,'Dominos','{kID}','false')";
                SqlCommand pizzaekleme = new SqlCommand(query, con.connection);
                pizzaekleme.ExecuteNonQuery();
                MessageBox.Show("Ürün Sepetinize eklenmiştir");
                con.connection.Close();
