...
{
    public partial class Hatay : Form
    {
        public string kadi;
        public string sifre;
        public Hatay(string kadii,string sifree)
        {
            InitializeComponent();
            kadi = kadii;
            sifre = sifree;
        ...
        private void btn_siparis_whopper_Click(object sender, EventArgs e)
        {
            ...
            con.connection.Open();
            string query = $"insert into Sepetim(Ürün,Fiyat,DukkanAdi,KullaniciID,Siparismi)values('Soslu Kıtır Tavuk Dürüm',85,'Hatay Kıtır Döner','{kID}','false')";
        ...
        private void btn_siparis_bigking_Click(object sender, EventArgs e)
        {
            ...
            con.connection.Close();
            con.connection.Open();
            string query = $"insert into Sepetim(Ürün,Fiyat,DukkanAdi,KullaniciID,Siparismi)values('Kıtır Kaşarlı Tavuk Dürüm',90,'Hatay Kıtır Döner','{kID}','false')";
            SqlCommand pizzaekleme = new SqlCommand(query, con.connection);
            pizzaekleme.ExecuteNonQuery();
            MessageBox.Show("Ürün Sepetinize eklenmiştir");
            con.connection.Close();
        ...
        private void btn_siparis_kingbeef_Click(object sender, EventArgs e)
        {
            ...
            con.connection.Close();
            con.connection.Open();
            string query = $"insert into Sepetim(Ürün,Fiyat,DukkanAdi,KullaniciID,Siparismi)values('Sushi Tavuk Dürüm',100,'Hatay Kıtır Döner','{kID}','false')";
            SqlCommand pizzaekleme = new SqlCommand(query, con.connection);
            pizzaekleme.ExecuteNonQuery();
            MessageBox.Show("Ürün Sepetinize eklenmiştir");
            con.connection.Close();
